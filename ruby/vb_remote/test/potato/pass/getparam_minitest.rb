require_relative '../minitest_helper'

class GetParamsFloat < Minitest::Test
    def test_it_gets_strip_0to4_mono_value_on
        (0..4).each do |num|
            @@vmr.set_parameter("Strip[#{num}].mono", ON)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Strip[#{num}].mono")
            assert_equal(ON, @@vmr.val)
        end
    end

    def test_it_gets_strip_0to4_mono_value_off
        (0..4).each do |num|
            @@vmr.set_parameter("Strip[#{num}].mono", OFF)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Strip[#{num}].mono")
            assert_equal(OFF, @@vmr.val)
        end
    end

    def test_it_gets_strip_0to4_solo_value_on
        (0..4).each do |num|
            @@vmr.set_parameter("Strip[#{num}].solo", ON)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Strip[#{num}].solo")
            assert_equal(ON, @@vmr.val)
        end
    end

    def test_it_gets_strip_0to4_solo_value_off
        (0..4).each do |num|
            @@vmr.set_parameter("Strip[#{num}].solo", OFF)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Strip[#{num}].solo")
            assert_equal(OFF, @@vmr.val)
        end
    end

    def test_it_gets_strip_0to4_mute_value_on
        (0..4).each do |num|
            @@vmr.set_parameter("Strip[#{num}].mute", ON)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Strip[#{num}].mute")
            assert_equal(ON, @@vmr.val)
        end
    end

    def test_it_gets_strip_0to4_mute_value_off
        (0..4).each do |num|
            @@vmr.set_parameter("Strip[#{num}].mute", OFF)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Strip[#{num}].mute")
            assert_equal(OFF, @@vmr.val)
        end
    end

    def test_it_gets_bus_0to7_mute_value_on
        (0..7).each do |num|
            @@vmr.set_parameter("Bus[#{num}].mute", ON)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Bus[#{num}].mute")
            assert_equal(ON, @@vmr.val)
        end
    end

    def test_it_gets_bus_0to7_mute_value_off
        (0..7).each do |num|
            @@vmr.set_parameter("Bus[#{num}].mute", OFF)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Bus[#{num}].mute")
            assert_equal(OFF, @@vmr.val)
        end
    end

    def test_it_gets_bus_0to7_mono_value_on
        (0..7).each do |num|
            @@vmr.set_parameter("Bus[#{num}].mono", ON)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Bus[#{num}].mono")
            assert_equal(ON, @@vmr.val)
        end
    end

    def test_it_gets_bus_0to7_mono_value_off
        (0..7).each do |num|
            @@vmr.set_parameter("Bus[#{num}].mono", OFF)
            assert_equal(SUCCESS, @@vmr.ret)
            @@vmr.get_parameter("Bus[#{num}].mono")
            assert_equal(OFF, @@vmr.val)
        end
    end
end

class GetParamsString < Minitest::Test
    def test_it_gets_strips_0to4_label_values
        (0..4).each do |num|
            [
                "testing[0]",
                "testing[1]",
                "testing[2]",
                "reset"
            ].each do |label|
                @@vmr.set_parameter("Strip[#{num}].Label", label)
                assert_equal(SUCCESS, @@vmr.ret)
                @@vmr.get_parameter("Strip[#{num}].Label")
                assert_equal(label, @@vmr.val)
            end
        end
    end
end
